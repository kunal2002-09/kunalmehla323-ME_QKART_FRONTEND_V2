...
router.get("/emails", verifyAuth, (req, res) => {
  console.log(`GET request received to "/user/emails"`);

  return res.status(200).json(req.user.emails);
});
...
// route to post emails in db
router.post("/emails", verifyAuth, (req, res) => {
  console.log(`POST request received to "/cart/emails"`);
  ...
  req.user.emails.push({
    _id: nanoid(),
    email: req.body.email,
  });
  users.update(
    { _id: req.user._id },
    { $set: { emails: req.user.emails } },
    {},
    (err) => {
      if (err) {
        handleError(res, err);
      ...
      console.log(
        `Email "${req.body.address}" added to user ${req.user.username}'s Email list`
      );
...
// router to delete the email
router.delete("/emails/:id", verifyAuth, async (req, res) => {
  console.log(`DELETE request received to "/cart/emails"`);

  const index = await req.user.emails.findIndex(
    (element) => element._id === req.params.id
  );
  if (index === -1) {
    return res.status(404).json({
      success: false,
      message: "Email to delete was not found",
    });
  }
  req.user.emails.splice(index, 1);
  users.update(
    { _id: req.user._id },
    { $set: { emails: req.user.emails } },
    {},
    (err) => {
      ...
      console.log(
        `Email with id ${req.user._id} deleteed from user ${req.user.username}'s emails list`
      );
