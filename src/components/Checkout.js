  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange={(e)=>handleNewAddress((prevAdd)=>({...prevAdd,value: e.target.value}))}
      />
      
      <Stack direction="row" my="1rem">
        <Button
          variant="contained" onClick={()=>addAddress(token,newAddress)}
        >
          Add
        </Button>
        <Button onClick={()=>handleNewAddress((prevAdd)=>({...prevAdd,isAddingNewAddress:false}))}
          variant="text"
        >
          Cancel
        </Button>
...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if(validationRequest) {
      ...
      try {
        const res = await axios.post(`${config.endpoint}/cart/checkout`,{"addressId":`${addresses.selected}`},{
        headers:{
          Authorization : `Bearer ${token}`,
        }
      })
      ...
      // localStorage.setItem({...localStorage, "balance": "balance" = (balance - getTotalCartValue(items)) })
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Divider />
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
                ...
                {addresses.all && addresses.all.map((address)=>{
                  return <Box onClick={()=>setAddresses({...addresses,selected:address._id})} className={address._id === addresses.selected ? "address-item selected" : "address-item not-selected"}>
                    <Typography my="1rem"  >
                 {address.address}
               </Typography>
            ...
            <Button
              ...
              variant="contained" onClick={()=>performCheckout(token,items,addresses)}
            >
              PLACE ORDER
            </Button>
          </Box>
