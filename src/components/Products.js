...
const Products = () => {
  ...
  const debounceSearch = (event, debounceTimeout) => {
    ...
    const timeout = setTimeout( () => performSearch(event.target.value)
    , 500);
    ...
    <div>
     ...
     <TextField
            ...
            {productsList.map((product) => {
              ...
              return (
                <Grid item md={3} xs={6} key={product._id}>
                  <ProductCard product={product} key={product._id} handleAddToCart={async () => await addToCart(window.localStorage.getItem('token'), cartItem, productsList, product._id, 1, {preventDuplicate: true})}/>
              </Grid>
              );
            })}
          </>
      ...
      <Grid item md={3} sm={12} style={{backgroundColor: '#E9F5E1' }}>
        <Cart items={generateCartItemsFrom(cartItem,productsList)}  handleQuantity={(e,id)=>handleQuantity(e,id)}/>
